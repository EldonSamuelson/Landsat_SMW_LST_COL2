...
Example 2:
  This example shows how to get LST time series at (almost) anywhere in the world
  It corresponds to the method used to extract time series for comparison with station LST 
  as used in Ermida et al. (2020).
    
    
NOTED BUGS:
  'Line 40: d.points[c] is undefined' - graph will freeze and no longer update.
   Current workaround is to not activate trendlines, or re-run script.
*/
...
Map.onClick(function(coords) {
    ...
    var geometry = ee.Geometry.Point(coords.lon, coords.lat).buffer(30);

print('Please wait while the chart is generated for ' + location);
...
var TimeSeries = ui.Chart.image.series(
    LandsatColl, geometry, ee.Reducer.mean(), 30, 'system:time_start')
        ...
        .setOptions({
          title: 'Surface Temperature of ' + location,
          vAxis: {  // y-axis
        ...
        pointSize: 4,
        /*trendlines: {
          // Draw a trendline for data series X (0-6).
          0: { // Series 2 at full time (1982-present) usually means Landsat 7 or 8
            type: 'polynomial',
            degree: 3 ,
            visibleInLegend: true,
            labelInLegend:'LST Series 1 Polynomial Trend'
          },
          1: { // Series 1 usually is Landsat 5 or 7
            visibleInLegend: true,
            labelInLegend: 'LST Series 1 Trendline'
          },
          }*/
});
